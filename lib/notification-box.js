...
exports.NotificationBox = function (options) {
    ...
    let mainBrowserWindow = getWindow();
    let tab = mainBrowserWindow.gBrowser.selectedBrowser;

    if (options.browserWindow && options.tab){
        tab = options.tab;
        mainBrowserWindow = options.browserWindow;
    }


    if (options.allTabs == true){
        var browsers = mainBrowserWindow.gBrowser.browsers;
        for (var i= 0; i<browsers.length; i++) {
            let nb = mainBrowserWindow.gBrowser.getNotificationBox(browsers[i]);
            BuildNotifications(options, nb);
        }
    } else {
        let nb = mainBrowserWindow.gBrowser.getNotificationBox(tab);
        BuildNotifications(options, nb);
    }
...
exports.hasNotification = function (browserWindow, tab){
    var notificationBox = browserWindow.gBrowser.getNotificationBox(tab);
    var notification = notificationBox.currentNotification;
    if(notification)
        return {value: notification.value, label:notification.label}
    else return null;
}
exports.clearNotification = function (value){
    var windows = winUtils.windows();
    for each(var window in windows){
        if(window.gBrowser)
            for each(var tab in window.gBrowser.browsers){
                let nb = window.gBrowser.getNotificationBox(tab);
...
let getWindow = function () {
    return winUtils.getMostRecentBrowserWindow();
};
